 ...
 */
package com.kang.bravedns.ui.fragment
...
import com.kang.bravedns.database.Severity
import com.kang.bravedns.databinding.FragmentHomeScreenBinding
import com.kang.bravedns.net.doh.Transaction
import com.kang.bravedns.scheduler.WorkScheduler
import com.kang.bravedns.service.BraveVPNService
import com.kang.bravedns.service.DnsLogTracker
import com.kang.bravedns.service.DomainRulesManager
import com.kang.bravedns.service.EventLogger
import com.kang.bravedns.service.FirewallManager
import com.kang.bravedns.service.IpRulesManager
import com.kang.bravedns.service.PersistentState
import com.kang.bravedns.service.ProxyManager
import com.kang.bravedns.service.VpnController
import com.kang.bravedns.service.WireguardManager
import com.kang.bravedns.service.WireguardManager.WG_HANDSHAKE_TIMEOUT
import com.kang.bravedns.service.WireguardManager.WG_UPTIME_THRESHOLD
import com.kang.bravedns.ui.activity.AlertsActivity
import com.kang.bravedns.ui.activity.AppListActivity
import com.kang.bravedns.ui.activity.ConfigureRethinkBasicActivity
import com.kang.bravedns.ui.activity.ConfigureRethinkBasicActivity.Companion.RETHINK_BLOCKLIST_NAME
import com.kang.bravedns.ui.activity.ConfigureRethinkBasicActivity.Companion.RETHINK_BLOCKLIST_URL
import com.kang.bravedns.ui.activity.CustomRulesActivity
import com.kang.bravedns.ui.activity.DnsDetailActivity
import com.kang.bravedns.ui.activity.FirewallActivity
import com.kang.bravedns.ui.activity.NetworkLogsActivity
import com.kang.bravedns.ui.activity.PauseActivity
import com.kang.bravedns.ui.activity.ProxySettingsActivity
import com.kang.bravedns.ui.activity.WgMainActivity
import com.kang.bravedns.ui.bottomsheet.HomeScreenSettingBottomSheet
import com.kang.bravedns.util.Constants
import com.kang.bravedns.util.Constants.Companion.RETHINKDNS_SPONSOR_LINK
import com.kang.bravedns.util.NotificationActionType
import com.kang.bravedns.util.UIUtils
import com.kang.bravedns.util.UIUtils.htmlToSpannedText
import com.kang.bravedns.util.UIUtils.openNetworkSettings
import com.kang.bravedns.util.UIUtils.openUrl
import com.kang.bravedns.util.UIUtils.openVpnProfile
import com.kang.bravedns.util.Utilities
import com.kang.bravedns.util.Utilities.delay
import com.kang.bravedns.util.Utilities.getPrivateDnsMode
import com.kang.bravedns.util.Utilities.isAtleastN
import com.kang.bravedns.util.Utilities.isAtleastP
import com.kang.bravedns.util.Utilities.isAtleastR
import com.kang.bravedns.util.Utilities.isOtherVpnHasAlwaysOn
import com.kang.bravedns.util.Utilities.isPrivateDnsActive
    ...
    private fun updateUiWithDnsStates(dnsName: String) {
        ...
        if (VpnController.isOn()) {
            io {
                Logger.i(
                    LOG_TAG_UI,
                    "dnsCardStatus: start dnsName=$dns queryTransportId=$id preferredId=$preferredId " +
                        "localHostsEnabled=${appConfig.isLocalHostsEnabled()} braveMode=${appConfig.getBraveMode()} " +
                        "systemDns=${appConfig.isSystemDns()} smartDns=${appConfig.isSmartDnsEnabled()} " +
                        "(card only checks Go getDnsStatus(queryTransportId); Local DNS is applied in onQuery, independent)"
                )
                var failing = false
                for (attempt in 1..5) {
                    ...
                    Logger.d(
                        LOG_TAG_UI,
                        "dnsCardStatus: attempt $attempt/5 queryTransportId=$id status=$status " +
                            "(null => transport id mismatch or unset in Go resolver)"
                    )
                    if (status != null) {
                        failing = false
                        Logger.i(
                            LOG_TAG_UI,
                            "dnsCardStatus: ok after $attempt attempt(s) queryTransportId=$id status=$status"
                        )
                        uiCtx {
                ...
                Logger.w(
                    LOG_TAG_UI,
                    "dnsCardStatus: showing failed_using_default after 5 attempts queryTransportId=$id " +
                        "localHostsEnabled=${appConfig.isLocalHostsEnabled()}; " +
                        "Local DNS can still answer in BraveVPNService.onQuery; this UI reflects Go transport registration only"
                )
                uiCtx {
