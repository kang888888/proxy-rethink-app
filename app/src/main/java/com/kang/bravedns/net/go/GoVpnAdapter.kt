 ...
 */
package com.kang.bravedns.net.go
...
import com.kang.bravedns.data.AppConfig.Companion.FALLBACK_DNS_IF_NET_DNS_EMPTY
import com.kang.bravedns.data.AppConfig.TunnelOptions
import com.kang.bravedns.database.ConnectionTrackerRepository
import com.kang.bravedns.database.DnsCryptRelayEndpoint
import com.kang.bravedns.database.EventSource
import com.kang.bravedns.database.EventType
import com.kang.bravedns.database.ProxyEndpoint
...
import com.kang.bravedns.service.BraveVPNService.Companion.FIRESTACK_MUST_DUP_TUNFD
import com.kang.bravedns.service.BraveVPNService.Companion.NW_ENGINE_NOTIFICATION_ID
...
import com.kang.bravedns.service.RethinkBlocklistManager
import com.kang.bravedns.service.WireguardManager
import com.kang.bravedns.ui.activity.AntiCensorshipActivity
import com.kang.bravedns.ui.activity.AppLockActivity
import com.kang.bravedns.util.Constants
import com.kang.bravedns.util.Constants.Companion.MAX_ENDPOINT
import com.kang.bravedns.util.Constants.Companion.ONDEVICE_BLOCKLIST_FILE_TAG
import com.kang.bravedns.util.Constants.Companion.REMOTE_BLOCKLIST_DOWNLOAD_FOLDER_NAME
import com.kang.bravedns.util.Constants.Companion.RETHINKDNS_DOMAIN
import com.kang.bravedns.util.Constants.Companion.RETHINK_BASE_URL_MAX
import com.kang.bravedns.util.Constants.Companion.RETHINK_BASE_URL_SKY
import com.kang.bravedns.util.Constants.Companion.UNSPECIFIED_IP_IPV4
import com.kang.bravedns.util.Constants.Companion.UNSPECIFIED_IP_IPV6
import com.kang.bravedns.util.UIUtils.getAccentColor
import com.kang.bravedns.util.Utilities
import com.kang.bravedns.util.Utilities.blocklistDir
import com.kang.bravedns.util.Utilities.blocklistFile
...
import com.kang.bravedns.util.Utilities.tob
import com.kang.bravedns.util.Utilities.togb
import com.kang.bravedns.util.Utilities.togs
import com.kang.bravedns.util.Utilities.tos
import com.kang.bravedns.wireguard.Config
    ...
    suspend fun addTransport() {
        ...
        when (appConfig.getDnsType()) {
            ...
            AppConfig.DnsType.LOCAL_HOSTS -> {
                // no-op; local hosts is a layer on top of real DnsType, not a transport
            }
    ...
    suspend fun getDnsStatus(id: String): Long? {
        try {
            ...
            if (transport == null) {
                Logger.d(
                    LOG_TAG_VPN,
                    "$TAG dns status; no transport for id=$id (resolver.get returned null); home card may show fallback warning"
                )
                return null
            }
            if (tid == null || tid.tos() != id) {
                Logger.d(
                    LOG_TAG_VPN,
                    "$TAG dns status; not desired transport: requested=$id actual=${tid?.tos()} " +
                        "(resolver using default/fallback); return null. " +
                        "Note: Local DNS hosts are resolved in onQuery and do not change this transport id."
                )
                return null
            }
