 ...
 */
package com.kang.bravedns.data
...
import com.kang.bravedns.database.DnsCryptEndpoint
import com.kang.bravedns.database.DnsCryptEndpointRepository
import com.kang.bravedns.database.DnsCryptRelayEndpoint
import com.kang.bravedns.database.DnsCryptRelayEndpointRepository
import com.kang.bravedns.database.DnsLogRepository
import com.kang.bravedns.database.DnsProxyEndpoint
import com.kang.bravedns.database.DnsProxyEndpointRepository
import com.kang.bravedns.database.DoHEndpoint
import com.kang.bravedns.database.DoHEndpointRepository
import com.kang.bravedns.database.DoTEndpoint
import com.kang.bravedns.database.DoTEndpointRepository
import com.kang.bravedns.database.EventSource
import com.kang.bravedns.database.EventType
import com.kang.bravedns.database.ODoHEndpoint
import com.kang.bravedns.database.ODoHEndpointRepository
import com.kang.bravedns.database.ProxyEndpoint
import com.kang.bravedns.database.ProxyEndpointRepository
import com.kang.bravedns.database.RethinkDnsEndpoint
import com.kang.bravedns.database.RethinkDnsEndpointRepository
...
import com.kang.bravedns.service.TcpProxyHelper
...
import com.kang.bravedns.util.InternetProtocol.Companion.getInternetProtocol
import com.kang.bravedns.util.OrbotHelper
import com.kang.bravedns.util.PcapMode
...
internal constructor(
    ...
    private val hostsProfileRepository: com.kang.bravedns.database.HostsProfileRepository,
    private val localHostsResolver: com.kang.bravedns.service.LocalHostsResolver,
    ...
    init {
        migrateLocalDnsLayerFromLegacyDnsType()
    ...
    fun getDnsType(): DnsType {
        ...
        return when (persistentState.dnsType) {
            ...
            DnsType.LOCAL_HOSTS.type -> DnsType.SYSTEM_DNS
            else -> {
    ...
    /**
     * Older builds stored "Local DNS" as [DnsType.LOCAL_HOSTS]. That is now a separate flag
     * ([PersistentState.localDnsLayerEnabled]) so TYPE/upstream and local overlay are independent.
     */
    private fun migrateLocalDnsLayerFromLegacyDnsType() {
        if (persistentState.dnsType != DnsType.LOCAL_HOSTS.type) return
        persistentState.localDnsLayerEnabled = true
        persistentState.dnsType = DnsType.SYSTEM_DNS.type
    ...
    private suspend fun onDnsChange(dt: DnsType) {
        ...
        when (dt) {
            ...
            DnsType.LOCAL_HOSTS -> {
                ...
                val activeCount = hostsProfileRepository.getActiveProfiles().size
                postConnectedDnsName("Local Hosts ($activeCount)")
            }
        }
    }
    ...
    fun isLocalHostsEnabled(): Boolean {
        return persistentState.localDnsLayerEnabled
    }

    suspend fun enableLocalHosts() {
        val wasOff = !persistentState.localDnsLayerEnabled
        persistentState.localDnsLayerEnabled = true
        // Default profile + snapshot; safe if already on (e.g. user toggles quickly).
        localHostsResolver.ensureInitialized()
        val prev = persistentState.connectedDnsName
        onDnsChange(getDnsType())
        if (wasOff) {
            logEvent(
                EventType.DNS_SERVER_CHANGE,
                "Local DNS layer enabled",
                "Local DNS layer enabled, prev: $prev"
            )
        }
    }

    suspend fun disableLocalHosts() {
        if (!persistentState.localDnsLayerEnabled) return

        persistentState.localDnsLayerEnabled = false
        val prev = persistentState.connectedDnsName
        onDnsChange(getDnsType())
        logEvent(
            EventType.DNS_SERVER_CHANGE,
            "Local DNS layer disabled",
            "Local DNS layer disabled, prev: $prev"
        )
    }
