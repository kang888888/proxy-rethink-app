...
import com.kang.bravedns.data.AppConfig.Companion.DOH_INDEX
import com.kang.bravedns.data.AppConfig.Companion.DOT_INDEX
...
import com.kang.bravedns.databinding.FragmentDnsConfigureBinding
import com.kang.bravedns.scheduler.WorkScheduler
import com.kang.bravedns.scheduler.WorkScheduler.Companion.BLOCKLIST_UPDATE_CHECK_JOB_TAG
...
import com.kang.bravedns.ui.activity.DnsListActivity
import com.kang.bravedns.ui.activity.LocalHostsProfilesActivity
import com.kang.bravedns.ui.activity.PauseActivity
import com.kang.bravedns.ui.bottomsheet.DnsRecordTypesBottomSheet
import com.kang.bravedns.ui.bottomsheet.LocalBlocklistsBottomSheet
import com.kang.bravedns.util.NewSettingsManager
import com.kang.bravedns.util.UIUtils
import com.kang.bravedns.util.UIUtils.fetchColor
import com.kang.bravedns.util.UIUtils.setBadgeDotVisible
...
class DnsSettingsFragment : Fragment(R.layout.fragment_dns_configure),
    ...
    private fun initView() {
        ...
        showSplitDnsUi()
        updateAllowedRecordTypesUi()
        setLocalHostsSwitchSilently(isLocalHosts())
    }
    ...
    private fun updateLatency(dnsType: String = b.connectedStatusTitleUrl.text.toString()) {
        io {
            ...
            uiCtx {
                ...
                val merged = getString(R.string.two_argument_space, text, dnsType)
                b.connectedStatusTitleUrl.text = merged
            }
        }
    }
    ...
    private fun updateConnectedStatus(connectedDns: String) {
        ...
        if (WireguardManager.oneWireGuardEnabled()) {
            b.dcLocalDnsStatusRow.visibility = View.GONE
        ...
        when (appConfig.getDnsType()) {
            ...
            AppConfig.DnsType.LOCAL_HOSTS -> {
                dnsType = resources.getString(R.string.local_hosts)
                b.connectedStatusTitleUrl.text = resources.getString(R.string.local_hosts)
                b.connectedStatusTitle.text = dns
            }
        }
        b.queryCardViewTop.visibility = View.VISIBLE
        if (appConfig.isLocalHostsEnabled()) {
            b.dcLocalDnsStatusRow.visibility = View.VISIBLE
            b.dcLocalDnsStatusTitle.text = getString(R.string.local_hosts)
            b.dcLocalDnsStatusUrl.text = ""
        } else {
            b.dcLocalDnsStatusRow.visibility = View.GONE
        }
    ...
    private fun isLocalHosts(): Boolean {
        return appConfig.isLocalHostsEnabled()
    }
    ...
    private fun enableDnsRadios() {
        b.rethinkPlusDnsRb.isEnabled = true
        b.customDnsRb.isEnabled = true
        b.networkDnsRb.isEnabled = true
        b.smartDnsRb.isEnabled = true
        b.rethinkPlusDnsRb.isClickable = true
        b.customDnsRb.isClickable = true
        b.networkDnsRb.isClickable = true
        b.smartDnsRb.isClickable = true
        b.dcLocalHostsSwitch.isEnabled = true
        b.dcLocalHostsManageRl.isEnabled = true
        b.dcLocalHostsManageRl.isClickable = true
    }
    ...
    private fun disableAllDns() {
        ...
        b.dcLocalHostsSwitch.isEnabled = false
        b.dcLocalHostsManageRl.isEnabled = false
        b.dcLocalHostsManageRl.isClickable = false
    }

    private var suppressLocalHostsSwitchCallback = false

    private fun setLocalHostsSwitchSilently(checked: Boolean) {
        suppressLocalHostsSwitchCallback = true
        b.dcLocalHostsSwitch.isChecked = checked
        suppressLocalHostsSwitchCallback = false
    }
    ...
    private fun getConnectedDnsType(): String {
        return when (appConfig.getDnsType()) {
            ...
            AppConfig.DnsType.LOCAL_HOSTS -> {
                resources.getString(R.string.local_hosts)
            }
        }
    }
    ...
    private fun initClickListeners() {
        ...
        b.dcLocalHostsSwitch.setOnCheckedChangeListener { _: CompoundButton, isChecked: Boolean ->
            if (suppressLocalHostsSwitchCallback) return@setOnCheckedChangeListener
            io {
                if (isChecked) {
                    appConfig.enableLocalHosts()
                } else {
                    appConfig.disableLocalHosts()
                }
            }
        }

        b.dcLocalHostsManageRl.setOnClickListener {
            startActivity(Intent(requireContext(), LocalHostsProfilesActivity::class.java))
        }
