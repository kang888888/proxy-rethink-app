...
//import com.kang.bravedns.rpnproxy.RpnProxyManager.RpnMode
//import com.kang.bravedns.rpnproxy.RpnProxyManager.rpnMode
import Logger
...
import com.kang.bravedns.RethinkDnsApplication.Companion.DEBUG
import com.kang.bravedns.customdownloader.IpInfoDownloader
import com.kang.bravedns.data.AppConfig
import com.kang.bravedns.data.ConnTrackerMetaData
import com.kang.bravedns.data.ConnectionSummary
import com.kang.bravedns.database.AppInfo
import com.kang.bravedns.database.ConnectionTracker
import com.kang.bravedns.database.ConnectionTrackerRepository
import com.kang.bravedns.database.ConsoleLog
import com.kang.bravedns.database.EventSource
import com.kang.bravedns.database.EventType
import com.kang.bravedns.database.RefreshDatabase
import com.kang.bravedns.net.go.GoVpnAdapter
import com.kang.bravedns.net.manager.ConnectionTracer
import com.kang.bravedns.receiver.NotificationActionReceiver
import com.kang.bravedns.receiver.UserPresentReceiver
import com.kang.bravedns.scheduler.EnhancedBugReport
import com.kang.bravedns.service.FirewallManager.NOTIF_CHANNEL_ID_FIREWALL_ALERTS
import com.kang.bravedns.service.ProxyManager.ID_WG_BASE
import com.kang.bravedns.service.ProxyManager.isNotLocalAndRpnProxy
import com.kang.bravedns.ui.NotificationHandlerActivity
import com.kang.bravedns.ui.activity.AppLockActivity
import com.kang.bravedns.ui.activity.MiscSettingsActivity
import com.kang.bravedns.util.AndroidUidConfig
import com.kang.bravedns.util.BackgroundAccessibilityService
import com.kang.bravedns.util.BubbleHelper
import com.kang.bravedns.util.CoFactory
import com.kang.bravedns.util.ConnectivityCheckHelper
import com.kang.bravedns.util.Constants
import com.kang.bravedns.util.Constants.Companion.INIT_TIME_MS
import com.kang.bravedns.util.Constants.Companion.INVALID_UID
import com.kang.bravedns.util.Constants.Companion.NOTIF_INTENT_EXTRA_ACCESSIBILITY_NAME
import com.kang.bravedns.util.Constants.Companion.NOTIF_INTENT_EXTRA_ACCESSIBILITY_VALUE
import com.kang.bravedns.util.Constants.Companion.PRIMARY_USER
import com.kang.bravedns.util.Constants.Companion.UID_EVERYBODY
import com.kang.bravedns.util.CrashReporter
import com.kang.bravedns.util.Daemons
import com.kang.bravedns.util.IPUtil
import com.kang.bravedns.util.InternetProtocol
import com.kang.bravedns.util.KnownPorts
import com.kang.bravedns.util.MemoryUtils
import com.kang.bravedns.util.NotificationActionType
import com.kang.bravedns.util.OrbotHelper
import com.kang.bravedns.util.Protocol
import com.kang.bravedns.util.ResourceRecordTypes
import com.kang.bravedns.util.UIUtils.getAccentColor
import com.kang.bravedns.util.Utilities
import com.kang.bravedns.util.Utilities.isAtleastO
import com.kang.bravedns.util.Utilities.isAtleastQ
import com.kang.bravedns.util.Utilities.isAtleastR
import com.kang.bravedns.util.Utilities.isAtleastS
import com.kang.bravedns.util.Utilities.isAtleastU
import com.kang.bravedns.util.Utilities.isFdroidFlavour
import com.kang.bravedns.util.Utilities.isMissingOrInvalidUid
import com.kang.bravedns.util.Utilities.isNetworkSame
import com.kang.bravedns.util.Utilities.isPlayStoreFlavour
import com.kang.bravedns.util.Utilities.isUnspecifiedIp
import com.kang.bravedns.util.Utilities.showToastUiCentered
import com.kang.bravedns.util.Utilities.tos
import com.kang.bravedns.wireguard.WgHopManager
...
class BraveVPNService : VpnService(), ConnectionMonitor.NetworkListener, Bridge, OnSharedPreferenceChangeListener {
    ...
    private val localHostsResolver by inject<LocalHostsResolver>()
            ...
            PersistentState.DNS_CHANGE -> {
                ...
                io("dnsChange") {
                    when (appConfig.getDnsType()) {
                        ...
                        AppConfig.DnsType.LOCAL_HOSTS -> {
                            localHostsResolver.ensureInitialized()
                            setNetworkAndDefaultDnsIfNeeded(forceUpdate = true)
                        }
                    }
                }
    ...
    override fun onQuery(uidGostr: Gostr?, qdn: Gostr?, qtype: Long): DNSOpts = go2kt(dnsQueryDispatcher) {
        ...
        if (appConfig.isLocalHostsEnabled()) {
            // Refresh snapshot from DB so hosts/profile edits apply immediately (no VPN restart).
            localHostsResolver.reload()
        }
    ...
    private suspend fun checkUserAllowedDnsQtypes(result: DNSOpts, uid: Int, fqdn: String, qtype: Long, rinr: Boolean): DNSOpts {
        ...
        if (!ResourceRecordTypes.isQtypeAllowed(qtype.toInt(), allowedTypes)) {
            ...
            if (appConfig.isLocalHostsEnabled()) {
                val local = localHostsResolver.resolve(fqdn, qtype.toInt())
                if (local != null) {
                    val r = makeNsOpts(uid, Pair(Backend.System, ""), fqdn, false, rinr = rinr)
                    r.ipcsv = local.ip
                    Logger.vv(
                        LOG_TAG_VPN,
                        "onQuery: local-hosts hit for $fqdn, qtype: $qtype, profile: ${local.profileId}, src: ${local.source}, ip: ${local.ip}"
                    )
                    return r
                }
            }
