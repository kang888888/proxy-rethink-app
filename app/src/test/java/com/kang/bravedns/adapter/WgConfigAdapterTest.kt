...
import com.kang.bravedns.adapter.OneWgConfigAdapter.DnsStatusListener
import com.kang.bravedns.database.WgConfigFiles
import com.kang.bravedns.database.WgHopMap
...
@org.robolectric.annotation.Config(sdk = [28], shadows = [com.kang.bravedns.shadows.ShadowRouterStats::class])
class WgConfigAdapterTest : KoinTest {
        ...
        org.koin.core.context.startKoin {
            modules(
                module {
                    ...
                    single { mockk<com.kang.bravedns.database.WgConfigFilesRepository>(relaxed = true) }
                    single { mockk<com.kang.bravedns.database.WgHopMapRepository>(relaxed = true) }
    ...
    private fun createTestWgConfigFiles(
        ...
        every { wgConfig.oneWireGuard } returns oneWireGuard
    ...
    fun `test WireguardManager getConfigById with correct Int parameter`() {
        ...
        val mockConfig = mockk<com.kang.bravedns.wireguard.Config>(relaxed = true)
    ...
    fun `test WgConfigFiles creation with all properties`() {
        ...
        assertFalse("Expected oneWireGuard: false", config.oneWireGuard)
    ...
    @Test
    fun `test WgConfigFiles oneWireGuard configuration`() {
        val oneWgConfig = createTestWgConfigFiles(
            id = 1,
            isActive = false,
            oneWireGuard = true
        )

        assertFalse("Expected isActive: false", oneWgConfig.isActive)
        assertTrue("Expected oneWireGuard: true", oneWgConfig.oneWireGuard)
    }
    ...
    @Test
    fun `test adapter with oneWireGuard configurations`() = testScope.runTest {
        val oneWgCfg = createTestWgConfigFiles(
            id = 3,
            name = "OneWg Config",
            isActive = false,
            oneWireGuard = true
        ...
        mockViewHolder.update(oneWgCfg)
        verify { mockViewHolder.update(oneWgCfg) }
    }
    ...
    fun `test Amnezia configuration detection`() {
        ...
        val mockConfig = mockk<com.kang.bravedns.wireguard.Config>()
